    ...
    <div className="bg-[white] py-[12.5px] px-[20px] rounded-[16px] shadow-sm">
      ...
      <div
        className={`grid grid-cols-1 ${
          type === "customers" ? "md:grid-cols-4" : "md:grid-cols-3"
      ...
      >
        {overviewData.map(({ text, value }) => {
          return (
            <span
              key={text}
              className="border-[1px] text-center p-3 border-solid border-[#f5f5f5] rounded-[12px] bg-[white]"
            >
              <h3 className="text-[24px] text-vrBlack font-[600]">{value}</h3>

              <p className="text-[12px] text-[#606060] mt-5 font-[200]">
                {text}
              </p>
            </span>
          );
