...
export const ListingPlaceholder = (props: iPlaceholder) => {
  ...
  return (
    <div>
      ...
      {title.toLowerCase() !== "most watchlisted" && (
        <hr className="mt-1 mb-3 bg-[#f5f5f5] -z-3" />
      )}
      <div className="flex flex-row gap-2 items-center">
        <span
          className={`p-5 bg-[#f5f5f5] rounded-[12px] border-[1px] solid ${placeholderStyle}`}
        >
          <Image
        ...
        <h2 className="text-[20px] text-vrBlack font-[600]">
          {value === 0 && "None"}
        </h2>
      </div>
    </div>
    ...
    >
      ...
      <div className="flex flex-row items-center justify-between">
        <div className="overview__title flex flex-row items-center gap-2">
          <h2 className="text-[18px] text-vrBlack font-[600]">
            Listing Overview
          </h2>
        </div>
        ...
        <Link
          href={goToLink}
          className="text-[12px] text-[#cbcbcb] flex flex-row items-center gap-1"
        >
          <span>view all</span>
      ...
      <div className="grid grid-cols-2 justify-between mt-5">
        {data &&
          data.map(({ title, listingImage, value }, index) => {
            return (
              <div key={index} className="">
                <ListingPlaceholder
                  ...
                  title={title}
                  placeholderStyle={
                    index === 0 ? "border-vrLemon" : "border-vrBlue"
                  }
      ...
      <div className="bg-[#f5f5f5] p-4 rounded-[12px] grid grid-cols-1 md:grid-cols-2 items-end justify-between mt-6">
        <ListingPlaceholder
          value={value3}
          title={title3}
          placeholderStyle="border-vrYellow"
          listingImage={listingImage3}
        />
        <ListingWatchList
          watchListTitle={watchListTitle}
          watchListValue={watchListValue}
        />
      </div>
    </Box>
  );
