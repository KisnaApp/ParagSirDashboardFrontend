    ...
    <div className="grid grid-cols-2 gap-4 py-5">
      {SalesData.map(({ progress, amount, title, percent }, index) => {
        return (
          <div
            key={title}
            className="py-[8px] px-[15px] rounded-[12px] border-[1px] border-solid border-[#e4e4e4]"
          >
            <h3
            ...
            >
              {" "}
              ₦ {amount.split(".")[0]}
              <span className={index === 2 ? "opacity-[0.4]" : ""}>
                .{amount.split(".")[1]}
              </span>{" "}
            </h3>
            <span className="flex flex-row items-center gap-1 mt-2">
              ...
              <p
                className={`text-[10px] flex flex-row items-center gap-1 ${
                  progress === "up" ? "text-vrLemon" : "text-[#FF6A6A]"
                }`}
              >
                {SalesBalanceIcon[progress]} {percent}%
              </p>
            </span>
          </div>
        );
